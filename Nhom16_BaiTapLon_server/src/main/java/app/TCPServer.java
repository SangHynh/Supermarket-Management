		...
		try {
			...
			if (requestType.equals("LOGIN_REQUEST")) {
				...
				EntityManagerFactory entityManagerFactory = Persistence
						.createEntityManagerFactory("supermarket_server");
				...
				try {
					...
					// Xử lý ngoại lệ khi đọc dữ liệu từ client hoặc chuyển đổi dữ liệu không thành
					// công
					out.println("ADD_INVOICE_FAILED");
					e.printStackTrace();
				}
			...
			else if (requestType.equals("UPDATE_TOTAL_FOR_INVOICE")) {
			    try {
			        // Đọc thông tin từ client
			        Long invoiceId = Long.parseLong(in.readLine());
			        double total = Double.parseDouble(in.readLine());

			        // Gọi service để cập nhật tổng tiền cho hóa đơn
			        EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("supermarket_server");
			        InvoiceDAO invoiceDAO = new InvoiceDAO(entityManagerFactory.createEntityManager());
			        boolean isSuccess = invoiceDAO.updateTotalForInvoice(invoiceId, total);

			        // Gửi phản hồi về cho client
			        if (isSuccess) {
			            // Nếu cập nhật thành công, gửi thông báo về client
			            out.println("UPDATE_TOTAL_FOR_INVOICE_SUCCESS");
			        } else {
			            // Nếu cập nhật không thành công, gửi thông báo về client
			            out.println("UPDATE_TOTAL_FOR_INVOICE_FAILED");
			        }
			...
			else if (requestType.equals("DELETE_INVOICE_DETAIL")) {
			    try {
			        // Đọc thông tin từ client
			        long invoiceId = Long.parseLong(in.readLine());
			        int inventoryId = Integer.parseInt(in.readLine());
			        ...
			        boolean isSuccess = invoiceDetailDAO.deleteInvoiceDetailByInventoryIdAndInvoiceId(invoiceId, inventoryId);

			        // Gửi phản hồi về cho client
			        if (isSuccess) {
			            // Nếu xóa thành công, gửi thông báo về client
			            out.println("DELETE_INVOICE_DETAIL_SUCCESS");
			        } else {
			            // Nếu xóa không thành công, gửi thông báo về client
			            out.println("DELETE_INVOICE_DETAIL_FAILED");
			        }
			    } catch (IOException | NumberFormatException e) {
			        // Xử lý ngoại lệ khi đọc dữ liệu từ client hoặc chuyển đổi dữ liệu không thành công
			        out.println("DELETE_INVOICE_DETAIL_FAILED");
				...
				try {
					...
					InvoiceDetailDAO invoiceDetailDAO = new InvoiceDetailDAO(
							entityManagerFactory.createEntityManager());
					List<Object[]> invoiceDetails = invoiceDetailDAO
							.getInvoiceDetailsWithProductInfoByInvoiceId(invoiceId);
					...
					if (invoiceDetails != null && !invoiceDetails.isEmpty()) {
						...
						for (Object[] detail : invoiceDetails) {
							// Chi tiết hóa đơn gồm mã hóa đơn, mã sản phẩm, tên sản phẩm, đơn giá, số lượng
							// và tổng
			...
			else if (requestType.equals("GET_ALL_EMPLOYEES")) {
				try {
					// Gọi EmployeeDAO để lấy danh sách tất cả nhân viên từ cơ sở dữ liệu
					EntityManagerFactory entityManagerFactory = Persistence
							.createEntityManagerFactory("supermarket_server");
					EmployeeDAO employeeDAO = new EmployeeDAO(entityManagerFactory.createEntityManager());
					List<Employee> employees = employeeDAO.getAllEmployees();

					// Gửi phản hồi về cho client
					if (!employees.isEmpty()) {
						// Gửi kích thước của danh sách nhân viên
						out.println("EMPLOYEES_SIZE");
						out.println(employees.size());

						// Gửi từng nhân viên
						for (Employee employee : employees) {
							out.println(employee.getId()); // ID nhân viên
							out.println(employee.getName()); // Tên nhân viên
							out.println(employee.getPhone()); // Số điện thoại
							out.println(employee.isRole()); // Vai trò
							out.println(employee.getImage()); // Hình ảnh
							out.println(employee.getEmail()); // Email
							out.println(employee.isGender()); // Giới tính
							out.println(employee.getPassword()); // Mật khẩu
							out.println(employee.isStatus()); // Trạng thái
						}
					} else {
						// Gửi thông báo không có nhân viên
						out.println("NO_EMPLOYEES");
					}
				} catch (Exception e) {
					// Xử lý ngoại lệ khi gửi dữ liệu không thành công
					out.println("GET_EMPLOYEES_FAILED");
					e.printStackTrace();
				}
			}

			// thêm nhân viên
			else if (requestType.equals("ADD_EMPLOYEE")) {
				try {
					// Đọc dữ liệu về nhân viên từ client
					String name = in.readLine();
					...
					// Tạo một đối tượng Employee từ dữ liệu nhận được
					Employee newEmployee = new Employee(name, phone, role, image, email, gender, password, status);

					// Gọi EmployeeDAO để thêm nhân viên mới vào cơ sở dữ liệu
					EntityManagerFactory entityManagerFactory = Persistence
							.createEntityManagerFactory("supermarket_server");
					EmployeeDAO employeeDAO = new EmployeeDAO(entityManagerFactory.createEntityManager());
					...
					// Gửi phản hồi về cho client
					if (success) {
						out.println("ADD_EMPLOYEE_SUCCESS");
					} else {
						out.println("ADD_EMPLOYEE_FAILED");
					}
				} catch (Exception e) {
					// Xử lý ngoại lệ khi gửi dữ liệu không thành công
					out.println("ADD_EMPLOYEE_FAILED");
			...
			// cập nhật nhân viên
				...
				try {
					...
					// Tạo một đối tượng Employee từ dữ liệu nhận được
					Employee updatedEmployee = new Employee(id, name, phone, role, image, email, gender, status);

					// Gọi EmployeeDAO để cập nhật thông tin nhân viên trong cơ sở dữ liệu
					EntityManagerFactory entityManagerFactory = Persistence
							.createEntityManagerFactory("supermarket_server");
					EmployeeDAO employeeDAO = new EmployeeDAO(entityManagerFactory.createEntityManager());
					...
					// Gửi phản hồi về cho client
					if (success) {
						out.println("UPDATE_EMPLOYEE_SUCCESS");
					} else {
						out.println("UPDATE_EMPLOYEE_FAILED");
					}
				} catch (Exception e) {
					// Xử lý ngoại lệ khi gửi dữ liệu không thành công
					out.println("UPDATE_EMPLOYEE_FAILED");
			...
			//cập nhật mật khẩu
			else if (requestType.equals("UPDATE_PASSWORD_EMPLOYEE")) {
			    try {
			        // Đọc dữ liệu về nhân viên từ client
			        int id = Integer.parseInt(in.readLine());
			        String newPassword = in.readLine(); // Đọc mật khẩu mới từ client

			        // Gọi EmployeeDAO để cập nhật mật khẩu của nhân viên trong cơ sở dữ liệu
			        EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("supermarket_server");
			        EmployeeDAO employeeDAO = new EmployeeDAO(entityManagerFactory.createEntityManager());
			        boolean success = employeeDAO.updateEmployeePassword(id, newPassword);

			        // Gửi phản hồi về cho client
			        if (success) {
			            out.println("UPDATE_PASSWORD_EMPLOYEE_SUCCESS");
			        } else {
			            out.println("UPDATE_PASSWORD_EMPLOYEE_FAILED");
			        }
			    } catch (Exception e) {
			        // Xử lý ngoại lệ khi gửi dữ liệu không thành công
			        out.println("UPDATE_PASSWORD_EMPLOYEE_FAILED");
